...
class MT_Wide_and_Deep(object):
    ...
    def create_model(self, X, S_lengths, S_indices, T):
        ...
        print("Trying to run MT_Wide_and_Deep for the first time", file=run_log, flush=True)
        if T is not None:
        ...
        print("Ran MT_Wide_and_Deep for the first time", file=run_log, flush=True)
...
if __name__ == "__main__":
    ...
    print("Trying to initialized MT_Wide_and_Deep", file=run_log, flush=True)
    with core.DeviceScope(device_opt):
        mtwnd = MT_Wide_and_Deep( args )
    print("Initialized MT_Wide_and_Deep Net", file=run_log, flush=True)
    ...
    for batch in range(10, 1001, 10):
        ...
        for k in range(args.nepochs):
            for j in range(nbatches):
                ...
                time_load_end, time_finish = mtwnd.run(lX[j], lS_l[j], lS_i[j], args.enable_profiling) # args.enable_profiling
