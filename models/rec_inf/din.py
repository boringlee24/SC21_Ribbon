...
class DIN_Wrapper(object):
    ...
    def __init__(
        ...
        if accel_en:
            ...
            print("(Wrapper) Using {} Accel(s)...".format(naccels), file=run_log, flush=True)
        else:
            device_opt = core.DeviceOption(caffe2_pb2.CPU)
            print("(Wrapper) Using CPU...", file=run_log, flush=True)
...
class DIN_Net(object):
    ...
    def create_model(self, X, S_lengths, S_indices, T):
        ...
        print("Trying to run DIN for the first time", file=run_log, flush=True)
        if T is not None:
        ...
        print("Ran DIN for the first time", file=run_log, flush=True)
...
if __name__ == "__main__":
    ...
    if use_accel:
        device_opt = core.DeviceOption(caffe2_pb2.CUDA, 0)
        naccels = C.num_cuda_devices # 1
        print("Using {} Accel(s)...".format(naccels), file=run_log, flush=True)
    else:
        device_opt = core.DeviceOption(caffe2_pb2.CPU)
    ...
    if args.data_generation == "dataset":
        print("Error we have disabled this function currently....", file=run_log, flush=True)
        sys.exit()
        ...
        # target data
        print("Generating output dataset", file=run_log, flush=True)
        (nbatches, lT) = dc.generate_output_data()

    ### construct the neural network specified above ###
    print("Trying to initialize DIN", file=run_log, flush=True)
    with core.DeviceScope(device_opt):
        din = DIN_Net( args )
    print("Initialized DIN Net", file=run_log, flush=True)

    din.create(lX[0], lS_l[0], lS_i[0], lT[0])
    print("Created network", file=run_log, flush=True)

    ################## what is related to batch size ########################
    
    # len(lX[0]) = len(lT[0]) = batch size
    # len(lS_l[0][0]) = batch size
    # lS_i seems to be not related

    for batch in range(10, 1001, 10):
        dc.args.max_mini_batch_size = batch
        (nbatches, lX, lS_l, lS_i) = dc.generate_input_data()
        ...
        runtime_ms = []
        print("Running networks", file=run_log, flush=True)
        for k in range(args.nepochs):
            for j in range(nbatches):
                ...
                time_load_end, time_finish = din.run(lX[j], lS_l[j], lS_i[j], args.enable_profiling) # args.enable_profiling
                runtime_ms.append(round((time_finish - time_load_start)*1000,3))
                time.sleep(0.1) #TODO
        print('median runtime is', np.median(runtime_ms), 'ms', file=run_log, flush=True)

        model_name = args.model_name # this is set automatically
#        batch_size = args.max_mini_batch_size  #TODO: only for random data generation
